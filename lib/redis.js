...
function RedisStore(params = {}) {
  ...
  this.startRedis = async function () {
    try {
      ...
      loggerTracer.info(chalk.green(`Load redis by ${name}-${version} successfully!`));
      loggerFactory.info(`Redis connection has been end`, {
        requestId: `${requestId}`,
      });
    } catch (err) {
  ...
  this.stopRedis = async function () {
    try {
      loggerFactory.info(`Redis closed connection has been start`, {
        requestId: `${requestId}`,
      });
      await client.disconnect();
      loggerTracer.info(chalk.green(`Stop load redis by ${name}-${version} successfully!`));
      loggerFactory.info(`Redis closed connection has been end`, {
        requestId: `${requestId}`,
      });
    } catch (err) {
