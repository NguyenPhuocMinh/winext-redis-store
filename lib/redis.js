...
function RedisStore(params = {}) {
  ...
  this.startRedis = async function () {
    try {
      ...
      loggerFactory.info(`Redis connect complete on`, {
        requestId: `${requestId}`,
        args: `${URL_REDIS}`,
      });
  ...
  this.stopRedis = async function () {
    try {
      loggerTracer.info(chalk.green.bold(`Load stop redis by ${name}-${version} successfully!`));
